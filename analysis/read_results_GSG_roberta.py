...
from analysis.extract_statistics import extract_statistics
...
BASE_DIR = "GSG-roberta/results-c/"
OUT_FILES = [
    "FULL.csv",
    "ST-A.csv",
    "ST-A-FUSION-30-GLUE.csv",
    "st-a-splitL__vector_2_avg_d03-lr1e-2-GLUE.csv",
    "st-a-splitL__vector_avg_d03-lr1e-2-GLUE.csv",
    "st-a-splitL__vector_val015_d03-lr1e-2-GLUE.csv"
]
...
DIR_NAMES = [
    "full",
    "st-a",
    "st-a-fusion-FP16-30-GLUE",
    "st-a-splitL__vector_2_avg_d03-lr1e-2-GLUE",
    "st-a-splitL__vector_avg_d03-lr1e-2-GLUE",
    "st-a-splitL__vector_val015_d03-lr1e-2-GLUE"
]

print(len(OUT_FILES), len(DIR_NAMES))
extract_statistics(OUT_FILES, DIR_NAMES, MODEL_NAME, "gsg-2")
...
# shutil.rmtree("GSG-roberta-roberta/results-c/differences", ignore_errors=False)
os.makedirs("GSG-roberta/results-c/differences/", exist_ok=True)
os.makedirs("GSG-roberta/results-c/differences/csv", exist_ok=True)
os.makedirs("GSG-roberta/results-c/differences/plots/all", exist_ok=True)
os.makedirs("GSG-roberta/results-c/differences/plots/100", exist_ok=True)
os.makedirs("GSG-roberta/results-c/plots/all", exist_ok=True)
os.makedirs("GSG-roberta/results-c/plots/100", exist_ok=True)
os.makedirs("GSG-roberta/results-c/csv", exist_ok=True)

# This code is used to store the results of the various models in a dictionary
result_dict = {}
for file in os.listdir("GSG-roberta/results-c"):
    if file.endswith(".csv") and not any(exclude in file for exclude in EXCLUDE_LIST):
        result_dict[file.split(".")[0]] = pd.read_csv(
            os.path.join("GSG-roberta/results-c", file)
        )
...
for name, df in result_dict.items():
    ...
    df.to_csv(f"GSG-roberta/results-c/csv/{name.split(' (')[0]}_proc.csv", index=False)
    ...
    px.bar(
        ...
        f"GSG-roberta/results-c/plots/all/{name.split(' (')[0]}.png"
    )
    ...
    px.bar(
        ...
        f"GSG-roberta/results-c/plots/100/{name.split(' (')[0]}.png", scale=3
    )
...
px.bar(
    ...
    "GSG-roberta/results-c/plots/ALL_100.png",
    # sharper
    scale=3,
)
...
px.bar(
    ...
    "GSG-roberta/results-c/plots/ALL_100.html",
    # sharper
...
def plot_100(
    ...
    fig.write_html(f"GSG-roberta/results-c/plots/{name}_100.html", include_plotlyjs="cdn")
    fig.write_image(f"GSG-roberta/results-c/plots/{name}_100.png", width=1000, height=600)
    ...
    # Save as HTML
    fig.write_html(f"GSG-roberta/results-c/plots/{name}_100_diff.html", include_plotlyjs="cdn")

    # Save as PNG
    fig.write_image(f"GSG-roberta/results-c/plots/{name}_100_diff.png", width=1000, height=600)
