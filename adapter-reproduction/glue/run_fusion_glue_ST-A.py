...
def main():
    ...
    model.load_adapter(
        "runs/st-a/rte/bert-base-uncased/1", config=PfeifferConfig(), with_head=False
    )
    model.load_adapter(
        "runs/st-a/mrpc/bert-base-uncased/1", config=PfeifferConfig(), with_head=False
    )
    model.load_adapter(
        "runs/st-a/cola/bert-base-uncased/2", config=PfeifferConfig(), with_head=False
    )
    model.load_adapter(
        "runs/st-a/mnli/bert-base-uncased/2", config=PfeifferConfig(), with_head=False
    )

    model.load_adapter(
        "runs/st-a/qnli/bert-base-uncased/2", config=PfeifferConfig(), with_head=False
    )
    model.load_adapter(
        "runs/st-a/qqp/bert-base-uncased/4", config=PfeifferConfig(), with_head=False
    )
    model.load_adapter(
        "runs/st-a/sst2/bert-base-uncased/2", config=PfeifferConfig(), with_head=False
    )
    model.load_adapter(
        "runs/st-a/stsb/bert-base-uncased/3", config=PfeifferConfig(), with_head=False
    )

    adapter_setup = [["rte", "mrpc", "cola", "mnli", "qnli", "qqp", "sst2", "stsb"]]
