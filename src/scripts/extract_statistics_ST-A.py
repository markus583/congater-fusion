...
for task in ["mnli", "qqp", "sst2", "mrpc", "rte", "qnli", "stsb", "cola"]:
    ...
    subfolder = os.path.join("runs", "st-a", task, MODEL_NAME)
    ...
    eval_results_dirs = [os.path.join(d, "eval_results.json") for d in seed_dirs if os.path.isfile(os.path.join(d, "eval_results.json"))]
