    ...
    def __init__(
        ...
        **kwargs,
        ...
        if no_init_bias and "bias" not in kwargs:
            kwargs["bias"] = False
        LoRALayer.__init__(
            self, location_key, config, in_features, out_features, **kwargs
        )
...
class MergedLinear(LoRALayer, nn.Linear):
    ...
    def add_adapter(self, adapter_name: str, layer_idx: int) -> bool:
        ...
        if is_added:
            ...
            if any(lora.enable_lora):
                # Compute the indices
                lora.lora_ind = self.weight.new_zeros(
                    (self.out_features,), dtype=torch.bool
                ).view(len(lora.enable_lora), -1)
    ...
    def pad(self, x, lora, fill_value=None):
        ...
        result[:, lora.lora_ind] = x.reshape(
    ...
    def reset_adapter(self):
        ...
        if self.merged:
            ...
            if lora.r > 0 and any(lora.enable_lora):
                ...
                self.weight.data = lora.com_inv(
                    self.weight.data, T(self.pad(delta_w, lora))
                )
            self.merged = None
