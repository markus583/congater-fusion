...
from huggingface_hub.utils import (EntryNotFoundError, RepositoryNotFoundError,
                                   RevisionNotFoundError, hf_raise_for_status)
...
CONGOSITION_CONFIG_NAME = "congosition_config.json"
CONGOSITION_WEIGHTS_NAME = "pytorch_model_congosition.bin"
...
def parse_adapter_config_string(config_string: str) -> List[Tuple[str, dict]]:
    ...
    for config_string_chunk in config_string_chunks:
        ...
        try:
            ...
            try:
                # "'key':value'" --> "'key': 'value'"
                value = re.search(r"'(\w+)':(\w+)", kvs).group(2)
                kvs = kvs.replace(value, "'" + value + "'")
                config_kwargs = ast.literal_eval("{" + kvs + "}")
            except Exception:
                try:
                    kvs = kvs[1:]
                    ...
                    raise ValueError(f"Invalid adapter configuration '{kvs}' in '{name}'.")
...
def resolve_adapter_config(
    config: Union[dict, str], local_map=None, try_loading_from_hub=True, **kwargs
) -> dict:
    """
...
def pull_from_hub(
    ...
    **kwargs,
    ...
    if not version:
        ...
        logger.warn(
            "Version '{}' of adapter '{}' not found. Falling back to default.".format(
                version, specifier
            )
        )
        version = hub_entry["default_version"]
    ...
    download_path = download_cached(
        file_entry["url"], checksum=checksum, checksum_algo=checksum_algo, **kwargs
    )
...
def resolve_adapter_path(
    ...
    **kwargs,
    ...
    # path to a local folder saved using save()
        ...
        if isfile(join(adapter_name_or_path, WEIGHTS_NAME)) and isfile(
            join(adapter_name_or_path, CONFIG_NAME)
        ):
        ...
        raise ValueError(
            "Unable to identify {} as a valid module location.".format(
                adapter_name_or_path
            )
        )
    ...
    if source == "hf" or source is None:
        ...
        for model_info in all_hf_adapters_data:
            adapter_info = AdapterInfo(
                ...
                model_name=model_info.config.get("adapter_transformers", {}).get(
                    "model_name"
                )
