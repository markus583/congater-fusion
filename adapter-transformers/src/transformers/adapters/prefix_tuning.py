...
class FlatPrefixTuning(nn.Module, ModuleUtilsMixin):
    def __init__(
        ...
        self.control_trans = nn.Parameter(
            torch.randn(self.config.prefix_length * self.n_layers * 2 * self.input_size)
        )
...
class PrefixTuningShim(AdapterLayerBase, nn.Module):
    ...
    def single_forward(
        ...
        prefix_keys, prefix_values = context.prefix_states[adapter_name][
            self.location_key
        ][prefix_id]
        ...
        prefix_keys, prefix_values = adjust_tensors_for_parallel(
            key_states, prefix_keys, prefix_values
        )
        ...
        if attention_mask is not None:
            if (
                ...
                prefix_mask = torch.ones(
                    batch_size, 1, attention_mask.size(2), prefix_keys.size(2)
    ...
    def _pad_and_concat(self, max_prefix_length, outputs, invert_mask=True):
        ...
        for key_states, value_states, residual_input, attention_mask in outputs:
            ...
            key_states = F.pad(
                key_states, pad_size, "constant", self.config.pad_token_id
            )
            value_states = F.pad(
                value_states, pad_size, "constant", self.config.pad_token_id
            )
            ...
            if pad_length > 0:
                ...
                ), "Attention mask must be set for prefix tuning"
    ...
    def adapter_parallel(
        ...
        if not context.adapters_parallelized:
            ...
            residual_input = residual_input.repeat(
                self.config.adapters.active_setup.parallel_channels, 1, 1, 1
            )
            key_states = key_states.repeat(
                self.config.adapters.active_setup.parallel_channels, 1, 1, 1
            )
            value_states = value_states.repeat(
                self.config.adapters.active_setup.parallel_channels, 1, 1, 1
            )
            if attention_mask is not None:
                if (
                    ...
                    attention_mask = attention_mask.repeat(
                        self.config.adapters.active_setup.parallel_channels, 1
                    )
                else:
                    attention_mask = attention_mask.repeat(
                        self.config.adapters.active_setup.parallel_channels, 1, 1, 1
    ...
    def adapter_batchsplit(
        ...
        for i, adapter_block in enumerate(adapter_setup):
            ...
            inputs = {
                ...
                "attention_mask": attention_mask[split_idx_range]
                if attention_mask is not None
                else None,
                "invert_mask": invert_mask,
