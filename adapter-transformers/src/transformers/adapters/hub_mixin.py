...
class PushAdapterToHubMixin:
    ...
    def _save_adapter_card(
        ...
        **kwargs,
        ...
        if datasets_tag is not None:
            dataset_name = (
                f"[{datasets_tag}](https://huggingface.co/datasets/{datasets_tag}/)"
            )
            datasets.add(datasets_tag)
        if adapterhub_tag is not None:
            # adapterhub_tag overwrites datasets_tag
            dataset_name = (
                f"[{adapterhub_tag}](https://adapterhub.ml/explore/{adapterhub_tag}/)"
            )
        ...
        if hasattr(self, "heads") and adapter_name in self.heads:
            head_type = " ".join(
                self.heads[adapter_name].config["head_type"].split("_")
            )
        ...
        logger.info(
            'Saving adapter card for adapter "%s" to %s.', adapter_name, save_directory
        )
    ...
    def push_adapter_to_hub(
        ...
        # Commit and push
        logger.info(
            'Pushing adapter "%s" to model hub at %s ...', adapter_name, repo_id
        )
        with working_or_temp_dir(
            working_dir=local_path, use_temp_dir=use_temp_dir
        ) as work_dir:
            files_timestamps = self._get_files_timestamps(work_dir)
            # Save adapter and optionally create model card
            self.save_adapter(work_dir, adapter_name)
            if overwrite_adapter_card or not os.path.exists(
