...
class WandbCallback(TrainerCallback):
    ...
    def __init__(self):
        ...
        # log model
        if os.getenv("WANDB_LOG_MODEL", "FALSE").upper() in ENV_VARS_TRUE_VALUES.union(
            {"TRUE"}
        ):
            DeprecationWarning(
    ...
    def on_log(self, args, state, control, model=None, logs=None, **kwargs):
        ...
        if state.is_world_process_zero:
            ...
            # log model after each evaluation
            # check if key eval_loss exists in logs dict
            if "train/loss" in logs.keys():
                # add image to wandb, every on_log call, based on model parameters
                all_omegas = []
                all_betas = []
                ...
                if len(named_params) > 0:
                    if hasattr(
                        ...
                        logger.info("Logging Compositionality")
                        for i, layer in enumerate(range(12)):
                            if list(
                                model.base_model.encoder.layer[
                                    0
                                ].output.congosition_v1_layer.children()
                            )[0].config.sigmoid:
                                sigmoid_temperature = list(
                                    model.base_model.encoder.layer[
                                        0
                                    ].output.congosition_v1_layer.children()
                                )[0].config.sigmoid_temperature
                                print(sigmoid_temperature)
                        ...
                        ax.set_yticklabels(
                            named_params[0][0].split(".")[0].split(","), rotation=0
                        )

                        fig.tight_layout()
                        if not os.path.exists(args.output_dir + "/omegas"):
                            os.makedirs(args.output_dir + "/omegas")
                        plt.savefig(
                            args.output_dir + f"/omegas/omegas_{state.global_step}.png"
                        )
                        plt.close()
                        self._wandb.log(
                            {
                                "Compositionality": self._wandb.Image(
                                    args.output_dir
                                    + f"/omegas/omegas_{state.global_step}.png"
                                ),
                                "train/global_step": state.global_step,
                            }
                        )

                if len(named_params) == 2 and not "bias" in named_params[1][0]:
                    if hasattr(
                        named_params[1][1],
                        "data",
                    ):
                        ...
                        fig.tight_layout()
                        if not os.path.exists(args.output_dir + "/betas"):
                            os.makedirs(args.output_dir + "/betas")
                        plt.savefig(
                            args.output_dir + f"/betas/betas_{state.global_step}.png"
                        )
                        plt.close()
                        self._wandb.log(
                            {
                                "Additionality": self._wandb.Image(
                                    args.output_dir
                                    + f"/betas/betas_{state.global_step}.png"
                                ),
                                "train/global_step": state.global_step,
                            }
                        )
                elif len(model.base_model.shared_parameters) > 0:
                    logger.info("Logging Compositionality")
                    for (
                        name,
                        param,
                    ) in (
                        model.base_model.shared_parameters.named_parameters()
                    ):
                        params = (
                            torch.mean(
                                param,
                        ...
                        ax.set_yticklabels(
                            name.split(".")[0].split(","), rotation=0
                        )

                        # title
                        ax.set_title(current_name)
                        fig.tight_layout()
                        if not os.path.exists(args.output_dir + "/params"):
                            os.makedirs(args.output_dir + "/params")
                        plt.savefig(
                            args.output_dir
                            + f"/params/params-{current_name}_{state.global_step}.png"
                        )
                        plt.close()
                        self._wandb.log(
                            {
                                current_name: self._wandb.Image(
                                    args.output_dir
                                    + f"/params/params-{current_name}_{state.global_step}.png"
                                ),
                                "train/global_step": state.global_step,
...
class ClearMLCallback(TrainerCallback):
    ...
    def on_save(self, args, state, control, **kwargs):
        if self._log_model and self._clearml_task and state.is_world_process_zero:
            ...
            self._clearml_task.update_output_model(
                artifact_path, iteration=state.global_step, auto_delete_file=False
            )
